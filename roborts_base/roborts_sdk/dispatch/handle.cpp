...
Handle::Handle(std::string serial_port,int baudrate) {
  serial_port_ = serial_port;
  serial_baudrate_= baudrate;
  device_ = std::make_shared<SerialDevice>(serial_port_, serial_baudrate_);
...
bool Handle::Init(){
  ...
  LOG_INFO<<"serial baudrate: "<<serial_baudrate_;
  if (!protocol_->Init()) {
