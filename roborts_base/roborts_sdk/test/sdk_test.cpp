...
int main() {
  ...
  auto sub1=h->CreateSubscriber<p_chassis_info_t>(CHASSIS_CMD_SET,PUSH_CHASSIS_INFO,CHASSIS_ADDRESS,IPC_ADDRESS,func);
  auto sub2=h->CreateSubscriber<p_uwb_data_t>(CHASSIS_CMD_SET,PUSH_UWB_INFO,CHASSIS_ADDRESS,IPC_ADDRESS,func2);
  auto sub3=h->CreateSubscriber<p_gimbal_info_t>(GIMBAL_CMD_SET,PUSH_GIMBAL_INFO,GIMBAL_ADDRESS,BROADCAST_ADDRESS,func3);

  /*-----------Publisher Test-------------*/
  auto pub1 = h->CreatePublisher<p_chassis_speed_t>(CHASSIS_CMD_SET,CTRL_CHASSIS_SPEED,IPC_ADDRESS,CHASSIS_ADDRESS);
  ...
  pub1->Publish(chassis_speed);
   ...
   /*-----------Client Test-------------*/
  auto client1=h->CreateClient<chassis_mode_e,chassis_mode_e>(CHASSIS_CMD_SET,SET_CHASSIS_MODE,IPC_ADDRESS,CHASSIS_ADDRESS);
